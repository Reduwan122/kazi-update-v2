...
class PoultryRepository(
    ...
    suspend fun addStaff(
        ...
        val existing = _allStaff.value.any { it.name.equals(trimmedName, ignoreCase = true) }
        if (existing) {
            withContext(Dispatchers.Main) { onError("এই নামের স্টাফ ইতোমধ্যে রয়েছে") }
            return@withContext
        }
        ...
        try {
            ...
            val current = _allStaff.value.filter { it.id != id }
            _allStaff.value = (current + staff).distinctBy { it.id }.sortedBy { it.name }
            withContext(Dispatchers.Main) { onSuccess() }
        } catch (e: Throwable) {
            Log.e(TAG, "Error adding staff: ${e.message}", e)
            withContext(Dispatchers.Main) { onError(e.localizedMessage ?: "স্টাফ যোগ করা সম্ভব হয়নি") }
        }
    }
